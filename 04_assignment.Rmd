...
```{r, eval=FALSE}
bestsellers %>% 
  mutate(names = str_replace(author, "Alexander", "Alex"))
```
...
##### I would use str_detect for this and look for symbols using the character tables. For symbols, I would do str_detect patterns using “\W”, character classes for upper and lower case letters and quantifiers for digits. 
...
province <- tm_shape(NLD_muni) +
    tm_polygons("perc_men", 
                palette = "RdYlBu") +
    tm_facets(by = "province")

tmap_arrange(perc_men, province)
```
...
##### From the misleading axes reading, I don't think I still understand what the writer had intended to show with the graph of Florida gun violence. I don't know how to read it correctly but the wrong way. In addition, I thought not including 0 in the axis is interesting because it can change the conclusion / result so much. Whereas it may seem like there is a significant difference without including 0 in the x-axis, it's actually a very small difference when it is included. Also, the graph with a categorical x-axis is pretty bad. 
